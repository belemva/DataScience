...
m <- t(as.matrix(container[ , .(l, d, h, w)])) # multple bin
m
...
sn4d$container # matrix of bins l, d, h, w (weight limit)
...
# p is determined w.r.t each bin using algorithm
#in gbp4d_solver_dpp_prep_create_p

sn4q$it # matrix of items x, y, z, w (weight container is holding when fit it into container),
#l, d, h, w (weight of it itself)
#(x, y, z, w set into -1 when item not fit into bin)

sn4q$k  # indicator of which items are fitted into bin
sn4q$container # matrix of bins l, d, h, w (weight limit)
...
sn1d <- gbp1d_solver_dpp(p = v, w = ldhw[4L, ], c = 90000)
...
#Verifications
sum_weight=aggregate(results$w, by=list(Category=results$tid), FUN=sum)
Results$volume=results$l*results$d*results$h/1000000000

sum_volume=aggregate(Results$volume, by=list(Category=results$tid), FUN=sum)
Max_vol=12032*2350*2698/1000000000
print("VOLUME DEPASSE")
i=1
for (i in 1:nrow(sum_volume)){
    if (sum_volume[i,2]>Max_vol)
        print(i)
}
